...
module.exports = function (root, opts, pluginOpts) {
    ...
    var styleFiles = [];
    if (opts.stylesPath) {
        var fileList = fs.readdirSync(path.resolve(cwd, result(opts.stylesPath, opts, pluginOpts)));
        for(var i=0; i < fileList.length; i++) {
            if(path.extname(fileList[i]) === '.css') {
                styleFiles.push(fileList[i]);
            }
        }
        params.styleFiles = styleFiles;
    }
